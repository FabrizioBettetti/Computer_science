...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    pms = []
    for pad in pads:
        em = ps4b.EncryptedMessage(ciphertext.get_text())
        pm = em.decrypt_message(pad)
        words = pm.get_text().split()
        valid_words = 0
        for word in words:
            if is_word(word_list, word):
                valid_words += 1
        valid_word_counts.insert(0, valid_words)
        pms.insert(0, pm)
    if len(valid_word_counts) > 0:
        max_index = valid_word_counts.index(max(valid_word_counts))
        return pms[max_index]
