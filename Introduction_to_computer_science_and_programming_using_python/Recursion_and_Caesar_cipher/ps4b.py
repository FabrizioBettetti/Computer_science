...
class Message(object):
    def __init__(self, input_text):
        ...
        assert type(input_text) == str, 'Argument passed to input_text parameter in __init__ method must be a string'
    ...
    def shift_char(self, char, shift):
        ...
        assert type(char) == str, 'Argument passed to char parameter in shift_char method must be a string'
        assert len(char) == 1, 'Argument passed to char parameter in shift_char method must be a single character'
        assert ord(char) >= ord(' ') and ord(char) <= ord('~'), 'Argument passed to char parameter in shift_char method must be a single character with ASCII in the range [32, 126]'
        assert type(shift) == int, 'Argument passed to shift parameter in shift_char method must be an integer'
        
        # From [32, 126] to [0, 94] (subtract 32)
        # % 95 to remain in [0, 94]
        # Restore [32, 126] from [0, 94] (add 32)
        # 32 = ord(' '), 126 = ord('~')
        return chr((ord(char) - ord(' ') + shift) % (ord('~') - ord(' ') + 1) + 32)
    ...
    def apply_pad(self, pad):
        ...
        assert type(pad) == list, 'Argument passed to pad parameter in apply_pad method must be a list'
        assert len(pad) == len(self.message_text), 'Argument passed to pad parameter in apply_pad method must be a list with the same length of the message text'
        for e in pad:
            assert type(e) == int, 'Argument passed to pad parameter in apply_pad method must be a list of integers'
        
        text = ''
        for i in range(len(pad)):
            text += self.shift_char(self.message_text[i], pad[i])
        return text
...
class PlaintextMessage(Message):
    def __init__(self, input_text, pad=None):
        ...
        assert pad == None or type(pad) == list, 'Argument passed to pad parameter in __init__ method must be None or a list'
...
class EncryptedMessage(Message):
    ...
    def decrypt_message(self, pad):
        ...
        assert type(pad) == list, 'Argument passed to pad parameter in decrypt_message method must be a list'
        assert len(pad) == len(self.message_text), 'Argument passed to pad parameter in decrypt_message method must be a list with the same length of the message text'
        for e in pad:
            assert type(e) == int, 'Argument passed to pad parameter in decrypt_message method must be a list of integers'
        
        decrypto_pad = [-e for e in pad]
        return PlaintextMessage(self.apply_pad(decrypto_pad), pad)
