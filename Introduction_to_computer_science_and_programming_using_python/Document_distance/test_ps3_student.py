...
class TestPrepData(unittest.TestCase):
    def test_prep_hello_world(self):
        ...
        result = ds.text_to_list(load_text("hello_world.txt"))
        self.assertEqual(result, expected)

    def test_prep_1a(self):
        expected = a1
        result = ds.text_to_list(load_text("test1a.txt"))
        self.assertEqual(result, expected)
...
class TestTFIDF(unittest.TestCase):
    ...
    def test_tfidf_big(self):
        text_file = 'test1a.txt'
        text_files = ['test1a.txt', 'test1b.txt', 'test2a.txt', 'test2b.txt', 'test3a.txt']
        result = ds.get_tfidf(text_file, text_files)
        expected = tfidf_list
        self.assertListAlmostEqual(result, expected)
