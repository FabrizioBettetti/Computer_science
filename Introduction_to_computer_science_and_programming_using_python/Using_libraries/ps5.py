...
def filter(pixels_list, color):
    ...
    pixels_transformed_list = []
    t = make_matrix(color)
    for pixel in pixels_list:
        r = matrix_multiply(t, pixel)
        pixel_tuple = ()
        for n in r:
            pixel_tuple += (int(n),)
        pixels_transformed_list.append(pixel_tuple)
    return pixels_transformed_list
...
def reveal_bw_image(filename):
    ...
    return pix_to_img([extract_end_bits(1, pixel) * 255 for pixel in pixels], img.size, 'L')
...
def reveal_color_image(filename):
    ...
    pixels_extracted = []
    for pixel in pixels:
        pixel_extracted = extract_end_bits(3, pixel)
        channels = ()
        for channel in pixel_extracted:
            channels += (int(255 / 7 * channel),)
        pixels_extracted.append(channels)
    return pix_to_img(pixels_extracted, img.size, 'RGB')
...
def main():
    ...
    im.show()
    im.save('unhidden_hidden_1.bmp')

    im2 = reveal_image('hidden2.bmp')
    im2.show()
    im2.save('unhidden_hidden_2.bmp')
    
    draw_kerb('filtered_image_15.png', 'Fabrizio')
    draw_kerb('unhidden_hidden_1.bmp', 'Fabrizio')
    draw_kerb('unhidden_hidden_2.bmp', 'Fabrizio')
