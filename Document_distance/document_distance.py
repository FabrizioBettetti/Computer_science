...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    for k in freq_dict1:
        if k in freq_dict2:
            combined_freq = freq_dict1[k] + freq_dict2[k]
            if combined_freq > max_freq:
                max_freq = combined_freq
                l.clear()
                l.append(k)
            elif combined_freq == max_freq:
                l.append(k)
        else:
            freq = freq_dict1[k]
            if freq > max_freq:
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    ...
    d = get_frequencies(text_to_list(text))
    total = sum(d.values())
    for k in d:
        d[k] /= total
    return d
...
def get_idf(file_paths):
    ...
    * IDF is calculated as IDF(i) = log_10(total number of documents / number of
    ...
    for file_path in file_paths:
        text = load_file(file_path)
        l.extend(list(get_frequencies(text_to_list(text)).keys()))
    d = get_frequencies(l)
    for k in d:
        d[k] = math.log10(total / d[k])
    return d  
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    word_scores = list(tfidf_dict.items())
    word_scores.sort(key=lambda t: t[0])
    word_scores.sort(key=lambda t: t[1])
    return word_scores
