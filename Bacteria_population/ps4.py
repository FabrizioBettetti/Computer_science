...
class Patient(object):
    ...
    def update(self):
        ...
        for b in surviving_bacteria:
            try:
                offspring_bacteria.append(b.reproduce(population_density))
            except:
                pass
...
def simulation_without_antibiotic(num_bacteria,
    ...
    population_averages = []
    for t in timesteps:
        population_averages.append(calc_pop_avg(populations, t))

    make_one_curve_plot(timesteps, population_averages, 'Timestep', 'Average Population', 'Without Antibiotic')
...
def calc_pop_std(populations, t):
    ...
    sum = 0
    for trial in populations:
        sum += (trial[t] - average) ** 2
    return (sum / len(populations)) ** 0.5
...
def calc_95_ci(populations, t):
    ...
    return (mean, 1.96 * std / (len(populations)) ** 0.5)


mean, width = calc_95_ci(populations, 299)
print('Mean @t=299:', round(mean, 3))
print('95% confidence interval @t=299: [' + str(round(mean - width, 3)) + ', ' +  str(round(mean + width, 3)) + ']')
print()
...
def simulation_with_antibiotic(num_bacteria,
    ...
    for _ in range(num_trials):
        ...
        trial.append(patient.get_total_pop())
        resistant_trial.append(patient.get_resist_pop())
        for _ in timesteps[1:151]:
            patient.update()
            trial.append(patient.get_total_pop())
            resistant_trial.append(patient.get_resist_pop())
        patient.set_on_antibiotic()
        for _ in timesteps[151:]:
            patient.update()
            trial.append(patient.get_total_pop())
            resistant_trial.append(patient.get_resist_pop())
        populations.append(trial)
        resistant_pop.append(resistant_trial)
    
    population_averages = []
    resistant_pop_averages = []
    for t in timesteps:
        population_averages.append(calc_pop_avg(populations, t))
        resistant_pop_averages.append(calc_pop_avg(resistant_pop, t))

    make_two_curve_plot(timesteps, population_averages, resistant_pop_averages, 'Total', 'Resistant', 'Timestep', 'Average Population', 'With an Antibiotic')
